...
1. **Ground removal + point cloud clip** — the ground is removed by a **fitted plane plus a facet gate**, not a Z-height cut (the old fixed-height skip is gone). `fit_ground_plane()` does an iteratively trimmed least-squares fit of `z = ax + by + c` (trims BOTH tails — upper = structure on the ground, lower = sub-surface reconstruction noise; `quantile=0.5`, `low_quantile=0.05`), fitted **before** the polygon clip so the wider cloud keeps a ground majority. Points within `ground_clearance_m` (0.4 m) of the fitted plane are dropped; then keep only points inside `mission_area_wgs84`. After region growing, `_reject_ground_facets()` drops any facet that is **both near-horizontal AND within `ground_facet_clearance_m` (1.5 m) of the plane** — testing orientation as well as height is what keeps walls and lampposts (vertical, survive at any height) while killing tarmac. Why not a height cut: DJI perception clouds carry ~0.5 m of vertical ground noise regardless of target size (Mijande 0.52 m, the 2026-07-10 parking lot 0.54 m per 2×2 m cell), so no constant separates a short object from sloping ground. See `docs/superpowers/specs/2026-07-10-ground-plane-facade-fix-design.md`.
...
- **PSDK Custom Widgets are live-link only** — they render on Pilot 2's live-flight view while the aircraft is powered and linked, and are not visible from the pre-flight mission library screen. Verified 2026-05-25: widgets render *and are interactive* **only when the app runs as a managed DPK** — a raw `systemd`/exec binary does not surface them. A future "tap → fly" widget (`Action(START)`) therefore requires the DPK deployment.
- **The DJI-sanctioned on-drone automation path is Manifold + PSDK**: `DjiWaypointV3_UploadKmzFile(bytes, len)` uploads the KMZ directly to the aircraft, `DjiWaypointV3_Action(START|STOP|PAUSE|RESUME)` controls execution, and a Custom Widget on Pilot 2's live view gives the pilot a tap-to-fly button. Reference sample: `samples/sample_c/module_sample/waypoint_v3/` in the PSDK source. In this shape Pilot 2 is the monitoring surface, not the mission chooser — the pilot does not browse the KMZ from the library. The minimal, unavoidable on-drone hook is exactly this: the WaypointV3 upload + one custom **Fly widget** that triggers `Action(START)`. After the tap it is 100% DJI's normal autonomous flight. AeroScan's job is "augment the KMZ + upload it"; DJI flies it.
- **The fly-ready software chain is fully wired** (verified 2026-05-25 by code audit of `kmz_runner.c` + the rc-companion). End to end: `PING/STAT` readiness handshake → `AUGM` (RC sends mission intent + ICP cloud) → augment subprocess → `PRVW` preview downlink → `EXEC` (pilot approves in the rc-companion) → `DjiWaypointV3_UploadKmzFile` → `READY_TO_FLY` → pilot taps the Pilot-2 custom **Fly widget** → `DjiWaypointV3_Action(START)` → mission flies, with `RegMissionStateCallback`/`RegActionStateCallback` surfacing live progress to the floating window. `DjiWaypointV3_Init` + both callbacks are registered in `AeroscanKmzRunner_Init`; `HandleApprovalFrame` sets `s_pendingExecute` and the worker uploads. (A stale code comment saying "Phase 2.2 wires EXEC; for now nothing" is **wrong** — EXEC is wired; treat that comment as fixed-in-spirit.)
- **What `DjiWaypointV3_Action(START)` actually checks — and what it does NOT.** DJI's docs (`50.function-overview/10.advanced-function/50.waypoint-mission.md:164`) document only a **mission-VALIDITY check** on START ("it will first check the mission information uploaded by the user; if the check fails, the drone will not perform the mission … check the error code"). The docs do **not** state that WaypointV3 START runs battery / GPS / home / obstacle SAFETY pre-checks. The explicit "you must subscribe to and rigorously check battery / RTH / fail-safe / RTK / obstacle sensing" guidance (`40.flight-control.md:433`) is specifically about the **Joystick / manual PSDK flight-control path WITHOUT the RC — a different API**, not WaypointV3. So: AeroScan does **not** implement its own pre-flight safety checks; it relies on the FC's documented mission-validity gate plus the aircraft's standard autonomous-flight interlocks (RC present). **Whether those interlocks actually fire for a PSDK WaypointV3 START is undocumented and must be confirmed on the first real device test** — treat it as a GO/NO-GO item, not a guarantee. Surface any START refusal (the FC validity error code) to the pilot via HMS + the floating window. (Do not write that "DJI does all the safety checks" — that overstates the documented behaviour.)
- **The Manifold PSDK app is deployed as a DJI DPK package** (`dji_app_ctl install -i <file>.dpk`, no sudo), managed through Pilot 2 — DJI's recommended production path (the raw-exec / `systemd --user` path is dev-only and documented to terminate abnormally). The DPK installs to `/open_app/<appname>` and DJI auto-generates a root SYSTEM service (`WantedBy=multi-user.target`, runs as sandboxed user `app<appname>`, so `dji` can't read its logs — use Pilot Log Export or `journalctl -u <app>.service`). We do **not** set our app as the boot app; **Smart3DExplore** stays the boot app and the pilot manually switches to our app in Pilot after the scan (one DPK holds the E-Port at a time). The DJI-registered auth identity (`USER_APP_NAME="Payload test ap"`, `USER_APP_ID="183281"`) must not change — the DPK *display* name (`psdk-demo`) is cosmetic only. Full reference: `docs/architecture/manifold-deployment.md`. (Note: earlier docs claimed `dji` had no root so `systemd --user` couldn't auto-start — that was wrong; the dev service did survive a cold reboot. DPK is chosen for production posture + stability + widget support, not because auto-start was impossible.)
...
├── dji_perception/<n>/            (<n> is NOT always 1 — seen 1 and 2; take the newest chunk across subdirs)
...
- **Mesh source:** AeroScan can pull `mesh_binary_*.ply` directly from `/blackbox/the_latest_flight/dji_perception/<n>/` over SSH (LAN at depot, USB-tether to M4E debug port at `192.168.42.120` in the field — DJI documents this for PCs in `Payload-SDK-Tutorial/docs/en/40.manifold-quick-start/04.development-environment-setup/01.hardware-environment-setup.md` line 69; hot-plug not supported, cable before aircraft power-on).
- **Density vs KMZ cloud:** Manifold has the unfiltered cloud, ~18.8 M points across all chunks for a typical flight. The KMZ's `cloud.ply` is a curated/decimated version (~416 K points) — DJI's post-flight thumbnail, ~10 cm point spacing, sized for Pilot 2's 3D Tiles preview. The two are **not the same data shipped to two places — they're different things**. The Manifold has the raw perception scan (every point the on-drone scanner emitted), the KMZ has DJI's post-processed sparser version. Per-flight size on the Manifold scales with scan duration: flight0008 = 268 MB / 15 chunks, flight0014 = 1.4 GB / 64 chunks, flight0016 (Mijande) = 1.0 GB / 54 chunks, flight0019 = 350 MB / 25 chunks. The existing `facades_from_pointcloud_cgal` pipeline runs unchanged on the merged Manifold cloud (with a 10 cm voxel downsample for compute speed, not for transport) and produces ~2.8× more facades than from the same KMZ — denser source resolves smaller features (window sills, parapets, balcony details) that the decimated KMZ cloud has lost.
- **Frame alignment:** Manifold's perception coordinates are in a local meter-scale frame **anchored on takeoff and yaw-aligned to the aircraft's heading at takeoff**, not to the KMZ's ENU frame. Per-flight rotation around Z is needed to register the Manifold cloud into the KMZ frame. Verified on flight0016/Mijande: ICP point-to-point gives fitness 0.986, RMSE 0.468 m, total yaw 85.74°, translation effectively zero (~24 mm). Use Open3D `registration_icp` with `max_correspondence_distance ≈ 2 m` and a coarse-yaw seed (try 0/90/180/270°, pick the one whose centroid is closest to the KMZ's). `scripts/build_manifold_overlay_kmz.py` produces a synthetic overlay KMZ for visual verification (replaces `cloud.ply` inside a source KMZ with the registered Manifold cloud, leaves waypoints/polygon/gimbal commands byte-identical so two KMZs can be loaded side-by-side in the viewer).
...
- **Flight identification:** Use `/blackbox/the_latest_flight` (DJI-maintained symlink); don't try to track `flightNNNN` numbers. **Slot directory mtimes LIE** (verified 2026-07-10): DJI's ring-buffer bookkeeping touches slot dirs without writing flight data, so `ls -dt /blackbox/*` can rank an empty/stale slot as "newest." On 2026-07-10 `flight0065`/`flight0066` showed recent dir mtimes while every file inside was from 2026-06-12. Rank slots by **per-file** mtime (`find <slot> -type f -newermt <date>`), not by `ls -dt`. The `the_latest_flight` symlink itself was correct. Also: a live slot is still being written while you rsync it — re-pull after power-down.
- **Archiving a flight:** `scripts/pull_flight_archive.sh` pulls a **whole** `/blackbox` slot (including `dji_perception/<n>/mesh_binary_*.ply`) into `flight-archive/<date>/`. Prefer it for post-flight archiving. `scripts/pull_flight_debug.sh` only stages PSDK logs + mission-state and **hits a permissions wall on the DPK log** — the DPK runs as a sandboxed user and its log dir is group-`nvidia`, but `dji` is not in group `nvidia`, so that log is unreadable via the debug script.
- **`/blackbox` is a ~30-slot ring buffer that cycles** (verified 2026-05-25). Slots `flight0020`–`flight0049` get **reused** — slot numbers are **not chronological** (confirmed by mtimes, not slot index). **A new flight slot is created by an aircraft POWER-CYCLE, not by app updates** (a DPK install / rebuild does nothing to `/blackbox`). The perception mesh (`mesh_binary_*.ply`) exists for a slot only if a Smart3D Auto-Exploration scan ran in that session, and it is evicted as the buffer cycles. **Verified 2026-05-25: the previously-only mesh (`flight0019`) has been pruned — no flight currently has a mesh.** Operational rule (the intended flow): fly a Smart3D scan, then **immediately** run the RC/augment so the fresh mesh is used before the buffer churns it — do not do many reboots between scan and augment. See memory `manifold-mesh-volatile`.
