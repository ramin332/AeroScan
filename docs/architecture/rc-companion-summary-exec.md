...
| Widgets op Pilot 2 (live view) | **Opgelost (2026-05-25):** widgets verschijnen én zijn interactief **mits de app als DPK draait** (niet als ruwe `systemd`-binary). De eigen **AeroScan "Fly"-widget** is nu gebouwd en de tap → `DjiWaypointV3_Action(START)`-handler is bedraad (`kmz_runner.c`, gated op `READY_TO_FLY`); mission/action-state-callbacks zijn geregistreerd. Nog onbewezen: de eerste echte WaypointV3-upload + START op de M4E (nooit gedraaid — er bereikte nooit een mesh de stap). |
...
2. ~~**Pilot 2 widget UI verkennen**~~ **Opgelost (2026-05-25):** de boord-app is nu als **DPK** geïnstalleerd (`dji_app_ctl install -i <file>.dpk`, beheerd via Pilot 2) en widgets renderen interactief op de live view. De ruwe `systemd`-binary deed dat niet. De eigen **"Fly"-widget** is gebouwd en bedraad naar `Action(START)`. Deployment-model: `manifold-deployment.md`. Resteert: de eerste echte WaypointV3-upload + START op de M4E (de GO/NO-GO — heeft eerst een mesh nodig).
